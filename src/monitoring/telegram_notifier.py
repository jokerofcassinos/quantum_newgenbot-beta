...
📈 Direção: <b>{trade.get('direction', 'BUY')}</b>
...
⚠️ Risco: <b>${trade.get('risk_amount', 0):,.2f}</b>
...
━━━━━━━━━━━━━━━━━━━━
💵 <b>P&L Líquido: ${pnl:+,.2f}</b>
📊 Pontos: {trade.get('pnl_points', 0):+,.0f}
⏱️ Duração: {trade.get('duration_minutes', 0)} min
📝 Exit: {trade.get('exit_reason', 'Unknown')}

💸 Comissão: ${trade.get('commission', 0):,.2f}
"""
        self.send_message(message.strip())
...
⚠️ Daily Loss: ${alert.get('daily_loss', 0):,.2f}
...
💸 Comissões: ${report.get('total_commissions', 0):,.2f}
...
⏰ {datetime.now(timezone.utc).strftime('%H:%M:%S UTC')}
⚠️ <b>ATENÇÃO IMEDIATA REQUERIDA</b>
"""
